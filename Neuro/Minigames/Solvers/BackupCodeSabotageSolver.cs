...
public sealed class BackupCodeSabotageSolver : IMinigameSolver<AuthGame, SabotageTask>, IMinigameOpener<SabotageTask>
{
    ...
    public IEnumerator CompleteMinigame(AuthGame minigame, SabotageTask task)
    {
        do
        {
            ...
            {
                yield return InGameCursor.Instance.CoMoveTo(minigame.ControllerSelectable[number > 0 ? number - 1 : 9]);
                minigame.ClickNumber(number);
                yield return new WaitForSeconds(0.5f);
            }
            yield return InGameCursor.Instance.CoMoveTo(minigame.ControllerSelectable[minigame.ControllerSelectable.Count - 1]);
            minigame.Enter();
            yield return new WaitForSeconds(0.5f);
